...
let page = document.getElementsByTagName("span");
let wrap = document.getElementsByClassName("wrap")[0];
let currentStyleLeft = -600;
let currentPage = 0;
...
function pageLeft(){
	...
	if(currentPage > 0){
		...
		page[currentPage - 1].className = "on";
		currentPage--;
	}else{
		page[currentPage].className = "";
		currentPage = 4;
		page[currentPage].className = "on";
	}

	//修改图片
	if(currentStyleLeft < -600){
		currentStyleLeft = currentStyleLeft + 600;
		wrap.style.left = currentStyleLeft + "px";
	}else{
		currentStyleLeft = -3000;
		wrap.style.left = currentStyleLeft + "px";
	}
}
...
function pageRight(){
	...
	//修改图片
	if(currentStyleLeft > -3000){
		currentStyleLeft = currentStyleLeft - 600;
		wrap.style.left = currentStyleLeft + "px";
	}else{
		currentStyleLeft = -600;
		wrap.style.left = currentStyleLeft + "px";
	}
}

arrowLeft.addEventListener('click',pageLeft);
arrowRight.addEventListener('click',pageRight);
...
//控制循环播放的变量已经在全局变量的部分声明
//设置对应鼠标事件
container.onmouseenter = function () {
    clearInterval(judge);
}
container.onmouseleave = function () {
    judge = setInterval("pageRight()",2000); 
}
