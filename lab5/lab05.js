...
function showWindowHref(){
    let url_str = url.value.split("&");//将url拆开
    url_str[0] = url_str[0].substr(url_str[0].indexOf("?")+1,url_str[0].length-url_str[0].indexOf("?"));//删除数组第一个元素中无关的字符串
    let url_judge = false;
    let url_output;

    for(let i=0;i<url_str.length;i++){
        let str_ele = url_str[i].split("=");
        if(str_ele[0] === "name"){
            url_judge=true;
            url_output = str_ele[1];
        }
    }

    if(url_judge){
    	url_result.value = url_output;
    }else{
    	url_result.value = "没有找到name参数";
    }
}
...
function arrSameStr(){
    ...
    for(let i =0;i < most.value.length;i++){
    	...
    	for(let j =0;j < most.value.length;j++){
            if(most.value[i] === most.value[j]){
            	num++;
            }
    	}
    	if(num > mostNum){
    		mostNum = num;
    		mostChar = most.value[i];
    	}
    }

    result.value = "The most character is:" + mostChar + " times:" + mostNum;
}
